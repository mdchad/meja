            ...
            .filter_map(|id| {
                ...
                match cmd_stat.status {
                    CommandStatus::STARTED => {
                        unreachable!(
                            "Only finished commands should be inserted into `slowest_commands`"
                        )
                    }
                    CommandStatus::FAILED(CommandStatusFailed { time_taken, .. }) => {
                        Some(FinishedCommandInfo::new(
                            time_taken,
                            cmd_stat.name.clone(),
                            cmd_stat.command.clone(),
                        ))
                    }
                    CommandStatus::SUCCESSFUL(CommandStatuSuccessful { time_taken, .. }) => {
                        Some(FinishedCommandInfo::new(
                            time_taken,
                            cmd_stat.name.clone(),
                            cmd_stat.command.clone(),
